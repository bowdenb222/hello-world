...
/* ── Where state lives ────────────────────────────────────────────────────
   Two very different homes, picked at runtime:

   'local'    localStorage. Correct for the installed PWA, where the page is a
              first-party origin and writes stick.
   'artifact' The artifact runtime's own file store. A preview link runs the
              page in a sandboxed cross-origin frame where localStorage throws
              outright, so nothing browser-side can persist there. Saving a data
              file alongside the page does persist, and survives across devices.
   'none'     Neither is available — say so rather than losing work quietly.

   The artifact runtime only answers asynchronously and never during the first
   synchronous run, so the page always renders from local state first and
   upgrades if and when the capability resolves. */
let STORE = 'local';
let artifactNS = null;
let publishing = false, publishErr = null, lastSaved = 0, userActed = false;
let pubTimer = null;
const DATA_FILE = 'data/garden.json';
...
if (!STORAGE_OK) STORE = 'none';

/* Only the parts worth carrying between sessions — not transient UI like the
   crop search box. */
function persistable(){
  return { region:S.region, last:S.last, first:S.first, plotW:S.plotW, plotD:S.plotD,
           season:S.season, rows:S.rows, history:S.history, plans:S.plans, planId:S.planId };
}
function adopt(r){
  if (!r || typeof r !== 'object') return false;
  S = Object.assign(DEFAULTS(), r);
  if (!S.rows || typeof S.rows !== 'object') S.rows = DEFAULTS().rows;
...
function save(){
  if (STORAGE_OK){
    ...
    catch(e){ STORAGE_OK = false; if (STORE === 'local') STORE = 'none'; }
  }
  if (artifactNS && userActed) schedulePublish();
  drawStorageWarning();
}

/* Every publish mints a version, so batch: a few seconds after the user stops. */
function schedulePublish(){
  clearTimeout(pubTimer);
  pubTimer = setTimeout(doPublish, 2500);
}
function flushPublish(){
  if (pubTimer){ clearTimeout(pubTimer); pubTimer = null; doPublish(); }
}
async function doPublish(){
  if (!artifactNS || publishing) return;
  publishing = true; drawStorageWarning();
  try {
    await artifactNS.publish({ [DATA_FILE]: JSON.stringify(persistable()) });
    lastSaved = Date.now(); publishErr = null;
  } catch (e) {
    const code = (e && e.code) || 'upstream_error';
    if (code === 'conflict'){
      publishErr = null;            /* the view is already reloading to the winner */
    } else if (code === 'rate_limited'){
      publishErr = null;
      setTimeout(schedulePublish, 15000);
    } else {
      publishErr = code;
    }
  } finally {
    publishing = false; drawStorageWarning();
  }
}
...
async function initArtifactStore(){
  ...
  let ns = null;
  try { ns = await window.claude.use('artifact'); } catch(e){ return; }
  if (!ns) return;
  artifactNS = ns;
  STORE = 'artifact';
  try {
    const res = await fetch(DATA_FILE, { cache:'no-store' });
    if (res.ok && adopt(await res.json())) renderAll();
  } catch(e){/* nothing saved yet, or the file is unreadable — carry on */}
  drawStorageWarning();
}

const ERRTEXT = {
  not_writer:'This is a read-only view of someone else\'s page, so changes are not kept. Open your own copy to save plans.',
  not_granted:'This view cannot save. Use Copy my data under Guide to keep a backup.',
  consent_required:'This view cannot save. Use Copy my data under Guide to keep a backup.',
  not_declared:'Saving is switched off for this page. Use Copy my data under Guide to keep a backup.',
  capability_disabled:'Saving is not available in this view. Use Copy my data under Guide to keep a backup.',
  capability_removed:'Saving is not available in this view. Use Copy my data under Guide to keep a backup.',
  too_large:'Too much saved data to store. Delete a plan you no longer need, then try again.',
  invalid_content:'The saved data could not be written. Use Copy my data under Guide to keep a backup.',
  read_only_path:'The saved data could not be written. Use Copy my data under Guide to keep a backup.',
  transform_error:'The saved data could not be written. Use Copy my data under Guide to keep a backup.',
  upstream_error:'Saving failed just now. It will retry on your next change; Copy my data under Guide is the safe backup.'
};
...
function drawStorageWarning(){
  ...
  if (STORE === 'artifact'){
    if (publishErr){
      host.innerHTML = `<div class="alert bad"><b>Not saving</b>${
        ERRTEXT[publishErr] || ERRTEXT.upstream_error}</div>`;
    } else {
      host.innerHTML = `<p class="savedline">${
        publishing ? 'Saving\u2026'
        : lastSaved ? 'Saved \u00b7 your plans stay on this link, on any device'
        : 'Changes save to this link automatically'}</p>`;
    }
    return;
  }
  if (STORE === 'local'){ host.innerHTML = ''; return; }

  host.innerHTML = `<div class="alert bad">
    <b>This browser will not save your work</b>
    Saved data is blocked here, so your plans disappear when you close the tab. Two
...
/* A debounced save must not be lost to a backgrounded or closed tab. */
